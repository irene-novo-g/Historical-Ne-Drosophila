...
#script_GONE_scratch.sh <d> <FOLDER> <FILE>
#To be used only in a machine with /state/partition1 directory

rm script_GONE_scratch.sh.*
...
mkdir -p $WDIR/estimNe_results/$FOLDER.n100.exp$EXP.m$mig.L$LAMB.K$NCRO.s$AVEs.h$AVEh/GONE/$FOLDER.$FILE/
DIR="estimNe_results/$FOLDER.n100.exp$EXP.m$mig.L$LAMB.K$NCRO.s$AVEs.h$AVEh"
fi
if [[ "$FOLDER" != BPcircular ]] ; then
mkdir -p $WDIR/estimNe_results/$FOLDER.n100.exp$EXP.L$LAMB.K$NCRO.s$AVEs.h$AVEh/GONE/$FOLDER.$FILE/
DIR="estimNe_results/$FOLDER.n100.exp$EXP.L$LAMB.K$NCRO.s$AVEs.h$AVEh"
fi
...
cp $WDIR/GONE/INPUT_PARAMETERS_FILE $SCDIR/$SLURM_JOBID/
