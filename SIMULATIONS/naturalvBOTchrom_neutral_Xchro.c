...
int NIND, CRO, CROX, KCRO, NCRO, NLOCI, TOTLOCI, block, model_selection;
int numberofrecom[NN], gen, generations, i, j, k, l, m;
int totalmutant[NN], doublemutant[NN], recurrentmutant[NN];
int mutants_ocurred, RM[NN], initialgen[MMM][31], dom, neutral;
...
headings()
{
	...
	fprintf(fgen,"\n\n N=%d    PS=%4.2f    L=%f   N.S-LOCI=%d    N.N-LOCI=%d   CHROM=%d  CRO.X=%d   gens=%d\n Lambda_a=%6.4f   Lambda_L=%6.4f   ave_aL=%6.4f    Psi=%6.4f   threshold=%6.4f   rho=%6.4f   Vs=%6.4f \n ave_s=%6.4f   PP_s=%6.4f   beta_s=%6.4f   alpha_s=%6.4f  dom=%d   k_s=%6.4f   ave_hs=%6.4f\n ave_a=%6.4f   PP_a=%6.4f   beta_a=%6.4f   alpha_a=%6.4f   k_a=%6.4f   ave_ha=%6.4f\n", NIND, PS, L, TOTLOCI-NCRO, NCRO, CRO, CROX, generations, Lambda_a, Lambda_L, ave_aL, Psi, threshold, rho, Vs, ave_s, P_s, beta_s, alpha_s, dom, k_s, ave_hs, ave_a, P_a, beta_a, alpha_a, k_a, ave_ha);
    	fclose(fgen);
}
...
int gm[][MMM][2], initialgen[][31];
{
    ...
    /* NEUTRAL LOCI WITH POISSON (2NL) NEW MUTATIONS */
    ...
    for (k=0; k<NCRO; k++)
    {
		...
		initialgen[k][0] = (-99);
		q[k][0] = 0.0;
		s[k][0] = 0.0; hs[k][0] = 0.0;
		a[k][0] = 0.0; ha[k][0] = 0.0;
	    }
    }
...
double pm_s[];
{
	int EE[MMM], FF[MMM], markerrecom, p1, p2, family[NN], chr, fp, lp;
	double family_sum2=0.0, sq_pm_a, rnd;
	...
	for (i=0; i<NIND; i++)
	{
		...
		if (GENBOT==0) 
		{
			do { p1 = (int)(uniform()*NIND); } 	while (p1%2!=0);
		}
		else if (GENBOT==1)
		{
			if (gen == 10000)			 	do { p1 = (int)(uniform()*10); } 	while (p1%2!=0);
			else						do { p1 = (int)(uniform()*NIND); }	while (p1%2!=0);
		}
		else if (GENBOT==10)
		{
			if ((gen >= 9991) && (gen <= 10000)) 		do { p1 = (int)(uniform()*10); } 	while (p1%2!=0);
			else						do { p1 = (int)(uniform()*NIND); } 	while (p1%2!=0);
		}
		else if (GENBOT==20)
		{
			if ((gen >= 9981) && (gen <= 10000)) 		do { p1 = (int)(uniform()*10); } 	while (p1%2!=0);
			else						do { p1 = (int)(uniform()*NIND); } 	while (p1%2!=0);
		}
		else if (GENBOT==100)
		{
			if ((gen >= 9901) && (gen <= 10000)) 		do { p1 = (int)(uniform()*100); } 	while (p1%2!=0);
			else						do { p1 = (int)(uniform()*NIND); } 	while (p1%2!=0);
		}
		...
		if ((PS==99)||(PS==0))
		{
			...
			{
				/* male */
				do { p2 = (int)(uniform()*NIND); }	while (p2%2==0);
			}
			else if (GENBOT==1)
			{
				if (gen == 10000)			 	do { p2 = (int)(uniform()*10); }	while (p2%2==0);
				else						do { p2 = (int)(uniform()*NIND); }	while (p2%2==0);
			}
			else if (GENBOT==10)
			{
				if ((gen >= 9991) && (gen <= 10000)) 		do { p2 = (int)(uniform()*10); }	while (p2%2==0);
				else						do { p2 = (int)(uniform()*NIND); }	while (p2%2==0);
			}
			else if (GENBOT==20)
			{
				if ((gen >= 9981) && (gen <= 10000)) 		do { p2 = (int)(uniform()*10); }	while (p2%2==0);
				else						do { p2 = (int)(uniform()*NIND); }	while (p2%2==0);
			}
			else if (GENBOT==100)
			{
				if ((gen >= 9901) && (gen <= 10000)) 		do { p2 = (int)(uniform()*100); }	while (p2%2==0);
				else						do { p2 = (int)(uniform()*NIND); }	while (p2%2==0);
			}
		}
		else
		...
		if (tracelevel!=0)	fprintf (fptr,"p1=%d\tp2=%d\n", p1, p2);
...
//					if (tracelevel!=0)   fprintf (fptr,"i=%d EE[0]=%d EE[1]=%d EE[2]=%d sm00=%d sm01=%d sm10=%d sm11=%d sm20=%d sm21=%d g00=%d g10=%d g20=%d \n", i, 			EE[0], EE[1], EE[2], sm[p1][0][0], sm[p1][0][1], sm[p1][1][0], sm[p1][1][1], 				sm[p1][2][0], sm[p1][2][1], gm[i][0][0], gm[i][1][0], gm[i][2][0]);
				...
				else
				{
					...
					for (k=0; k<NCRO; k++)
					{
						...
						/* no fathers gamete (X chromosome): gm[i][k][1] = mothers gamete for simplicity */
						gm[i][k][1]=((EE[k]&sm[p1][k][0])|(FF[k]&sm[p1][k][1]));
					}
//					if (tracelevel!=0)   fprintf (fptr,"i=%d EE[0]=%d EE[1]=%d EE[2]=%d sm00=%d sm01=%d sm10=%d sm11=%d sm20=%d sm21=%d g00=%d g10=%d g20=%d \n", i, 			EE[0], EE[1], EE[2], sm[p1][0][0], sm[p1][0][1], sm[p1][1][0], sm[p1][1][1], 				sm[p1][2][0], sm[p1][2][1], gm[i][0][0], gm[i][1][0], gm[i][2][0]);
				}
			}
		...
		{	/* ************** Restricted recombination ***************** */
			...
			for (k=0; k<NCRO; k++)
			{
				...
				for (l=0; l<NLOCI; l++)
				{
					if (marker==1)
					{
						if (uniform() < (L / (double)(TOTLOCI/CRO)))
						{
							EE[k] = EE[k] | RM[l];
			...
			for (chr=0; chr<CRO; chr++)
			{
				...
				for (k=fp; k<lp; k++)
				{
					...
					if ((CROX==1)&&(i%2!=0))
					{
						/* male offspring so no fathers gamete (X chromosome): gm[i][k][1] = mothers gamete for simplicity */
						gm[i][k][1]=((EE[k]&sm[p1][k][0])|(FF[k]&sm[p1][k][1]));
					}
				}
			}
			...
			for (chr=0; chr<CRO; chr++)
			{
				...
				for (k=fp; k<lp; k++)
				{
					...
					if (rnd < 0.5)	EE[k] = ~EE[k];
					FF[k] = ~EE[k];
					if (CROX==0)	gm[i][k][1]=((EE[k]&sm[p2][k][0])|(FF[k]&sm[p2][k][1]));
					else
					{
						if(i%2==0)	gm[i][k][1]=sm[p2][k][0];
		...
		if (tracelevel!=0) for (k=0; k<5; k++)	fprintf (fptr,"i%d k%d gm[i][k][0]=%d  gm[i][k][1]=%d   p1(mo): sm[p1][k][0]=%d  sm[p1][k][1]=%d   p2(fa): sm[p2][k][0]=%d  sm[p2][k][1]=%d\n", i, k, gm[i][k][0], gm[i][k][1], sm[p1][k][0], sm[p1][k][1], sm[p2][k][0], sm[p2][k][1]);
